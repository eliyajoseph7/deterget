            ...
            <div class="max-w-full mx-auto sm:px-6 lg:px-0">
                <div class="w-full">
                    <div class="flex justify-end pb-2 space-x-2">
                        <button
                            wire:click="$dispatch('openModal', {component: 'pages.rawmaterial.components.receive.receive-form'})"
                            class="items-center space-x-0.5 text-gray-600 hover:text-gray-500 bg-gray-50 hover:bg-white shadow-sm hover:shadow-md px-2 border-2 border-gray-100 mt-2 py-1 rounded-lg">
                            <i class="fa-solid fa-plus-circle"></i>
                            <span class="">{{ __('Receive Material') }}</span>
                        </button>
                        <a
                            href="{{ route('multiple_materials_receive') }}"
                            class="items-center space-x-0.5 text-gray-600 hover:text-gray-500 bg-gray-50 hover:bg-white shadow-sm hover:shadow-md px-2 border-2 border-gray-100 mt-2 py-1 rounded-lg">
                            <i class="fa-solid fa-plus-circle"></i>
                            <span class="">{{ __('Receive Multiple Material') }}</span>
                        </a>
                                    ...
                                    <table
                                        ...
                                        <thead
                                            ...
                                            <tr>
                                                ...
                                                <th scope="col" class="px-4 py-3">Measure</th>
                                            ...
                                            @forelse ($data as $dt)
                                                <tr wire:key="{{ $dt->id }}"
                                                    ...
                                                    {{-- <td class="px-4 py-3 whitespace-nowrap text-right">
                                                        {{ $dt->item?->unit_price }}</td> --}}
                                                    <td class="px-4 py-3 whitespace-nowrap">
                                                        {{ $dt->quantity }}</td>
                                                    <td class="px-4 py-3 whitespace-nowrap">
                                                        <a href="{{ $dt->invoice }}"
                                                            class="text-gray-400 hover:text-blue-500" target="_blank"
                                                            rel="noopener noreferrer">Invoice</a>
                                                    </td>
                                                    <td class="px-4 py-3 whitespace-nowrap">
                                                        {{ $dt->user?->name }}</td>
                                                    <td class="px-4 py-3 flex items-center justify-end space-x-1">
                                                        <button title="Update"
                                                            wire:click="$dispatch('openModal', {component: 'pages.rawmaterial.components.receive.receive-form', arguments: {id: {{ $dt->id }}}})"
