<div>
  <div class="relative">
    ...
    <div class="fixed top-40 right-8 z-50">
      <a href="{{ route('distributions') }}" class="bg-red-500 px-2 py-0.5 text-white rounded-md" title="close">x</a>
  </div>
  </div>
    <!-- Invoice -->
      ...
      <div class="flex flex-col p-4 sm:p-10 bg-white shadow-md rounded-xl dark:bg-neutral-800">
        ...
        <div class="flex justify-between">
          <div>
            ...
            <h1 class="mt-2 text-lg md:text-xl font-semibold text-blue-600 dark:text-white">Medics (T) Ltd.</h1>
          </div>
          <!-- Col -->
  
          <div class="text-end">
            <h2 class="text-2xl md:text-3xl font-semibold text-gray-800 dark:text-neutral-200">Invoice #</h2>
            <span class="mt-1 block text-gray-500 dark:text-neutral-500">{{ $invoiceno }}</span>
        ...
        <div class="mt-8 grid sm:grid-cols-2 gap-3">
          <div>
            ...
            <h3 class="text-lg font-semibold text-gray-800 dark:text-neutral-200">{{ $client->name }}</h3>
          ...
          <div class="sm:text-end space-y-2">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-1 gap-3 sm:gap-2">
              <dl class="grid sm:grid-cols-5 gap-x-3">
                ...
                <dd class="col-span-2 text-gray-500 dark:text-neutral-500">{{ $date->format('d/m/Y') }}</dd>
              </dl>
              {{-- <dl class="grid sm:grid-cols-5 gap-x-3">
        ...
        <div class="mt-6">
          <div class="border border-gray-200 p-4 rounded-lg space-y-4 dark:border-neutral-700">
            <div class="hidden sm:grid sm:grid-cols-5">
              ...
              <div class="text-start text-xs font-medium text-gray-500 uppercase dark:text-neutral-500">Price</div>
              <div class="text-end text-xs font-medium text-gray-500 uppercase dark:text-neutral-500">Amount (Tsh.)</div>
            </div>
            ...
            @foreach ($data as $dt)
              <div class="grid grid-cols-3 sm:grid-cols-5 gap-2">
                <div class="col-span-full sm:col-span-2">
                  ...
                  <p class="font-medium text-gray-800 dark:text-neutral-200">{{ $dt->product?->name }}</p>
                </div>
                <div>
                ...
                <div>
                  <h5 class="sm:hidden text-xs font-medium text-gray-500 uppercase dark:text-neutral-500">Price</h5>
                  <p class="text-gray-800 dark:text-neutral-200">{{ number_format($dt->product?->selling_price, 2) }}</p>
                </div>
                <div>
                  <h5 class="sm:hidden text-xs font-medium text-gray-500 uppercase dark:text-neutral-500">Amount (Tsh.)</h5>
                  <p class="sm:text-end text-gray-800 dark:text-neutral-200">{{ number_format($dt->quantity * $dt->product?->selling_price, 2) }}</p>
                </div>
              </div>
          ...
          <div class="w-full max-w-2xl sm:text-end space-y-2">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-1 gap-3 sm:gap-2">
              <dl class="grid sm:grid-cols-5 gap-x-3">
                ...
                <dd class="col-span-2 text-gray-500 dark:text-neutral-500">Tsh. {{ number_format($subtotal, 2) }}</dd>
              </dl>
  
              <dl class="grid sm:grid-cols-5 gap-x-3">
                <dt class="col-span-3 font-semibold text-gray-800 dark:text-neutral-200">VAT:</dt>
                <dd class="col-span-2 text-gray-500 dark:text-neutral-500">Tsh. {{ number_format($vat, 2) }}</dd>
              </dl>
  
              <dl class="grid sm:grid-cols-5 gap-x-3">
                <dt class="col-span-3 font-semibold text-gray-800 dark:text-neutral-200">Total:</dt>
                <dd class="col-span-2 text-gray-500 dark:text-neutral-500">Tsh. {{ number_format($total, 2) }}</dd>
              </dl>
